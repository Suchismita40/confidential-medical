...
export class MedExAPI implements DeployedMedExAPI {
    ...
    const indexerState$ = combineLatest(
      ...
      (ledgerState, privateState) => {
        ...
        return {
          ...
          datasetTitle: ledgerState.datasetTitle.is_some
            ? ledgerState.datasetTitle.value
            : undefined,
          datasetCategory: ledgerState.datasetCategory.is_some
            ? ledgerState.datasetCategory.value
            : undefined,
  ...
  private async executeTx(
    ...
    try {
      ...
      if (res && typeof res === "object" && "timeout" in res) {
        this.logger?.warn(
          `${actionName}: indexer confirmation timed out. Updating local state.`,
        );
      }
    } catch (err: unknown) {
      this.logger?.warn(
        { err },
        `Circuit call completed or timed out for ${actionName}`,
      );
    }
  ...
  async registerDataset(
    ...
    await this.executeTx(
      this.deployedContract.callTx.registerDataset(title, category),
      "registerDataset",
      {
        datasetTitle: title,
  ...
  async submitAccessProof(
    datasetId: Uint8Array,
    patientRecordHash: Uint8Array,
  ): Promise<void> {
    const currentState = this.internalState$.value;
    await this.executeTx(
      this.deployedContract.callTx.submitAccessProof(
        datasetId,
        patientRecordHash,
      ),
      "submitAccessProof",
  ...
  static async join(
    ...
    const deployedMedExContract = await findDeployedContract<MedExContract>(
      providers,
      {
        contractAddress,
        compiledContract: CompiledMedExContractContract,
        privateStateId: medexPrivateStateKey,
        initialPrivateState: await MedExAPI.getPrivateState(
          providers,
          contractAddress,
        ),
      },
  ...
  private static async getPrivateState(
    ...
    return (
      existingPrivateState ?? createMedExPrivateState(utils.randomBytes(32))
    );
  }
}
