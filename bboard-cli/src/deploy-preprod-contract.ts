...
const RESULT_FILE = path.resolve(
  currentDir,
  "..",
  "preprod-deployment-result.json",
);
const ROOT_RESULT_FILE = path.resolve(
  currentDir,
  "..",
  "..",
  "preprod-deployment-result.json",
...
async function main() {
  ...
  if (!verifiedOnIndexer) {
    throw new Error(`Double verification failed: contractAction for ${contractAddress} could not be resolved on indexer.`);
  }
  ...
  const uiEnvPreprod = path.resolve(
    currentDir,
    "..",
    "..",
    "bboard-ui",
