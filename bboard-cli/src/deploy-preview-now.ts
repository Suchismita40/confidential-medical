...
const SEED = process.env.PREVIEW_WALLET_SEED || process.env.WALLET_SEED;
...
async function main() {
  if (!SEED) {
    throw new Error('PREVIEW_WALLET_SEED or WALLET_SEED environment variable is required.');
  }
  ...
  if (dustBalance > 0n) {
    ...
    const result = {
      ...
      explorerUrl: `https://preview.midnightexplorer.com/contracts/${bboardApi.deployedContractAddress}`,
    ...
    console.log(`Explorer URL: https://preview.midnightexplorer.com/contracts/${bboardApi.deployedContractAddress}`);
    console.log('=============================');
  } else {
